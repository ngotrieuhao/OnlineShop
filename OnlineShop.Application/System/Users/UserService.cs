...
namespace OnlineShop.Application.System.Users
{
    public class UserService : IUserService
    {
        ...
        public async Task<ApiResult<string>> Authencate(LoginRequest request)
        {
            ...
            if (user == null) return new ApiErrorResult<string>(new string("Username does not exist"));

            // Trả về một SignInResult, tham số cuối là IsPersistent kiểu bool
            var result = await _signInManager.PasswordSignInAsync(user, request.Password, request.RememberMe, false);
            if (!result.Succeeded)
            {
                return new ApiErrorResult<string>(new string("Incorrect password"));
            }
        ...
        public async Task<ApiResult<string>> Register(RegisterRequest request)
        {
            ...
            if (user != null)
            {
                return new ApiErrorResult<string>(new string("This account has already existed"));
            }

            // Kiểm tra email đã tồn tại chưa
            if (await _userManager.FindByEmailAsync(request.Email) != null)
            {
                return new ApiErrorResult<string>(new string("Email already exists"));
            }
            ...
            {
                return new ApiErrorResult<string>(new string("Phone number already exists"));
            }
            ...
            return new ApiErrorResult<string>(new string("Registration failed"));
        }
        ...
        public async Task<ApiResult<bool>> ConfirmEmail(ConfirmEmailViewModel request)
        {
            ...
            if (user == null)
                return new ApiErrorResult<bool>($"User with Email not found {request.email}");
        ...
        public async Task<ApiResult<string>> ForgotPassword(ForgotPasswordViewModel request)
        {
            ...
            return new ApiErrorResult<string>($"Unable to recover password");
        }
